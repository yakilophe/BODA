...
(function () {
  ...
  set("dcTipo", D.dressCode.tipo);
  set("dcNota", D.dressCode.nota);
  pintarLista("dcDetalle", D.dressCode.detalle);
  pintarLista("tipsDetalle", D.tips);
  ...
  /* ---------------------------------------------------
     2. Enlaces de WhatsApp / formularios
     --------------------------------------------------- */
  const wa = (texto) =>
    "https://wa.me/" + String(D.whatsapp).replace(/\D/g, "") + "?text=" + encodeURIComponent(texto);

  const btnRsvp = $("#btnRsvp");
  if (btnRsvp) {
    btnRsvp.href = D.formularioRSVP ||
      wa("¡Hola! Confirmo mi asistencia a la boda de " + D.nombre1 + " y " + D.nombre2 + ". Vamos ___ persona(s). Mi nombre es: ");
  }

  const btnCancion = $("#btnCancion");
  if (btnCancion) {
    btnCancion.href = D.formularioCancion ||
      wa("¡Hola! Mi canción para la fiesta es: ");
  }
